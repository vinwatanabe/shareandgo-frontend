...
class User extends Component {
    ...
    getUserData = async() =>{
        try {
            ...
            const res = await axios.get(
                process.env.REACT_APP_BACKEND_URL + 'user/viewone/' + localStorage.getItem('userID'),
                {
                    headers: {
    ...
    updateUser = async (objUser) =>{
        try {
            ...
            const res = await axios.put(
                process.env.REACT_APP_BACKEND_URL + 'user/update/' + localStorage.getItem('userID'),
                objUser,
                {
                    headers: {
                        // Get user token from session storage
    ...
    async login (objUser) {
        try {
            // Send user object to server
            const res = await axios.post(process.env.REACT_APP_BACKEND_URL+"user/login", objUser);
            if(res.data.error) {
                // If error, send back error message
                console.log(res);
                return (res.data.error);
